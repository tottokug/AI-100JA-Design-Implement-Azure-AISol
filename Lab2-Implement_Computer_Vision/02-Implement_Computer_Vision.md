...
[メイン](https://github.com/MicrosoftLearning/AI-100-Design-Implement-Azure-AISol/tree/master/Lab2-Implement_Computer_Vision) github リポジト リフォルダーにはいくつかのディレクトリがあります。

- **sample_images**: Cognitive Services の実装のテストに使用するいくつかのサンプル画像。

- **code**: ここでは、2 つのディレクトリがあります。各フォルダーには、ラボの異なるプロジェクトを持つソリューション (.sln) が含まれています。
...
`ImageProcessing`ソリューション内には、`ProcessingLibrary`プロジェクトが含まれます。処理ライブラリは、いくつかのサービスのラッパーとして機能します。この特定の PCL には、Computer Vision API にアクセスするためのヘルパー クラス (ServiceHelpers フォルダー内) と、結果をカプセル化する "ImageInsights" クラスが含まれています。後で画像をラッピングし、Cognitive Services へのブリッジとして機能するいくつかのメソッドとプロパティを公開する画像プロセッサ クラスを作成します。
...
ご覧のように、画像から`キャプション`と`タグ`プロパティと、一意の`ImageId`が表示されます。"ImageInsights" は、Computer Vision API から情報を収集します。
...
```
1. おそらく、Project Oxford の NuGet パッケージをインストールする必要があるため、「**ツール**」メニューを選択します。
1. 次に、「**NuGet パッケージ マネージャー**」を強調表示し、「**ソリューションの Nuget パッケージの管理**」を選択します。
1. 「**検索**」ボックスに、「**ProjectOxford**」と入力します。
1. 一覧表示されているパッケージを選択し、プロジェクト用にインストールします
1. 「**ソリューション エクスプローラー**」で、**ImageStorageLibrary** プロジェクトの下の「**参照**」を展開し、**ProjectOxford** パッケージに黄色の注意マークが存在しなくなったことを確認します。
...
1. 「**ImageProcessor.cs**」で、まず画像の処理に使用するメソッド`ProcessImageAsync`を作成します。`ImageProcessor`クラス内 (`{ }`の間) に次のコードを貼り付けます。
...
1. Visual Studio で自動的に追加されない場合は、ファイルの最後に中括弧を追加してメソッドを閉じます
...
1. `ProcessImageAsync`の`{ }`の間に以下のコードを追加します。
...
Azure Cosmos DB はマイクロソフトの回復性の高い NoSQL PaaS ソリューションであり、画像メタデータの結果と同様に、緩やかに構造化されたデータを格納するのに非常に便利です。他にも選択肢 (Azure Table Storage、SQL Server) がありますが、Cosmos DB ではスキーマを自由に進化させ (新しいサービスへのデータの追加など)、簡単にクエリを実行し、Azure Search にすばやく統合するための柔軟性がもたらされます (後半のラボで実習)。	
...
> **注**: Azure の CosmosDb リソースで結果を参照することもできます。  リソースを開き、「**データ エクスプローラー**」を選択します。  **メタデータ** データベースを展開し、**項目**ノードを選択します。  結果を含む複数の json ドキュメントが表示されます。
